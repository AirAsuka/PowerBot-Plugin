...
- 每轮投票后进入夜晚。机器人私聊所有存活的普通拿词玩家，可选择“不刀”或指定目标 QQ。狼人开刀会杀死目标；平民开刀会导致自己出局。不刀指令不需要填写任何 QQ 号。
- 存活白板每夜有一次猜词机会，可提交两个词（顺序不限）或主动放弃。两个词同时猜中平民词和狼人词时，白板立即单独获胜；猜错后本夜不能重猜。天使没有夜间行动。
- 多名狼人的行动同时生效，可以选择不同目标；相同目标只出局一次。夜间行动超时为 2 分钟，普通玩家未提交按“不刀”处理，白板未提交按放弃猜词处理。
- 白板未触发独立胜利时和天使同属平民阵营。全部狼人出局时平民阵营获胜；存活狼人数达到其他存活人数时狼人阵营获胜。
...
“卧底刀人 不刀”永远不需要 QQ 号；如果同一玩家同时参与多个群，它会对全部待行动房间选择不刀。刀人或白板猜词只参与一个夜晚房间时可省略群号，多房间时可在参数前附加群号。
