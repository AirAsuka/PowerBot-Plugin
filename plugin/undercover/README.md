...
投票阶段会在每次投票反馈和“卧底状态”中列出已投票、未投票玩家。每次进入投票前，机器人会按轮次分别合并转发整局截至本轮的全部有效描述；第 2 轮及以后开始前，也会先按轮次分别合并转发此前所有轮次的描述，再播报全部死亡玩家及身份、全部存活玩家（不公开身份）。若平台不允许使用玩家身份创建转发节点，则改由机器人标注玩家昵称和 QQ 后合并转发。
