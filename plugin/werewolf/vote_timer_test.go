...
func newVotingGame(t *testing.T) *game {
	t.Helper()
	g := gameWithRoles(t, roleWolf, roleWolf, roleSeer, roleWitch, roleHunter, roleVillager)
	g.beginVoting()
	return g
}
...
func TestVotingTimeoutSettlesOnlyReceivedVotes(t *testing.T) {
	...
	if err != nil || !r.Complete || r.Eliminated != 6 || g.Players[6].Alive {
...
func TestVotingTimeoutTieStartsFreshDeadline(t *testing.T) {
	...
	if err != nil || !r.Complete || r.Eliminated != 3 {
