...
func shouldHandle(ctx *zero.Ctx) bool {
...
func resolveGID(ctx *zero.Ctx) int64 {
...
func buildRole(ctx *zero.Ctx) goba.PermissionRole {
...
func runAgentChat(ctx *zero.Ctx, gid int64, stor chat.Storage, temperature, topp, maxn float32, mp *syncx.Map[string, any]) bool {
...
func runNormalChat(ctx *zero.Ctx, gid int64, stor chat.Storage, temperature, topp, maxn float32) {
	...
	if len(txt) > 0 {
		...
		sendReply(ctx, gid, stor, txt, ctx.Event.IsToMe)
	}
}

func sendReply(ctx *zero.Ctx, gid int64, stor chat.Storage, txt string, replyToMe bool) {
