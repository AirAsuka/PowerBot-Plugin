...
func fetchMemeInfos() ([]MemeInfo, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("获取表情信息失败(HTTP %d): %s", resp.StatusCode, string(body))
	}
...
func parseMemeError(statusCode int, body []byte) error {
	...
	return fmt.Errorf("生成表情失败(HTTP %d): %s", statusCode, string(body))
}
...
func getImage(imageID string) ([]byte, error) {
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return nil, fmt.Errorf("获取图片失败(HTTP %d): %s", resp.StatusCode, string(body))
	}
...
func renderMemeList() ([]byte, error) {
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return nil, fmt.Errorf("渲染列表失败(HTTP %d): %s", resp.StatusCode, string(body))
	}
